    ...
    try:
        if display_menu == True:
            if enable_newswin == 'true':
                koding.Add_Dir(name='[B][COLOR red]Latest[COLOR lime] News [COLOR yellow]And [COLOR cyan]Updates[/COLOR][/B]', url='{"my_text":"[B][COLOR red]Latest[COLOR lime] News [COLOR yellow]And [COLOR cyan]Updates[/COLOR][/B]","my_desc":""}', mode='dialog_example', folder=False, icon=os.path.join(art_path,'icon.png'), fanart=os.path.join(art_path,'fanart.jpg'))
