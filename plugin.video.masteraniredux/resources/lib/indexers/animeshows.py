...
class Indexer:
    ...
    def get_popular(self):
        ...
        for i in result: self.list.append({'anime_id': i['slug'].split('-', 1)[0], 'status': 1, 'title': '%s - [I]%s views[/I]' %(i['title'], i['total']), 'poster':i['poster'], 'fanart':i['fanart'], 'plot': i['plot'], 'duration': None, 'type': 0, 'rating': None, 'genre': None, 'premiered': None, 'episode_count': i['episode_count'], 'slug': i['slug']})
    ...
    def get_being_watched(self):
        ...
        print result
        if len(result) is 0: return
        for i in result: self.list.append({'anime_id': i['slug'].split('-', 1)[0], 'status': 1, 'title': '%s - [I]%s People Watching[/I]' %(i['title'], i['total']), 'poster':i['poster'], 'fanart':i['fanart'], 'plot': i['plot'], 'duration': None, 'type': 0, 'rating': None, 'genre': None, 'premiered': None, 'episode_count': i['episode_count'], 'slug': i['slug']})
