...
def get_anime_details(anime_id):
    ...
    try:
        ...
        return ({'title': title, 'anime_id': anime_id, 'plot': plot, 'poster': poster, 'premiered': premiered, 'rating': rating, 'type': type, 'episode_count': episode_count, 'age_rating': age_rating, 'tvdb': tvdb, 'genre': genre, 'status': status, 'duration': duration, 'episodes': episodes2, 'fanart': fanart, 'showlink': link})
...
def extract_data_from_filter_list(data):
    ...
    for e in data['data']:
        ...
        mlist.append({'title': title, 'anime_id': anime_id, 'plot': plot, 'poster': poster, 'premiered': premiered,
             ...
             'genre': genre, 'status': status, 'duration': duration, 'fanart': fanart, 'slug': slug})
