...
def searchsubtitles(item):
    ...
    if item['mansearch']:
        ...
        if episodes:
            ...
            lists = search_links(search_string, sezon=sezon, episod=episod, item=item)
        else:
            if __addon__.getSetting("scrap") == "0":
                lists = search_links(search_string, item=item)
            else:
                lists = search_links(search_string, manual="1", item=item)
        return lists
    lists = search_links(search_string, item['year'], item['season'], item['episode'], item_orig, item=item)
    if not lists:
        if not item['file_original_path'].startswith('http') and xbmcvfs.exists(item['file_original_path']):
            ...
            lists = search_links(search_string, item['year'], item['season'], item['episode'], (re.compile(r'\[.*?\]').sub('', head)), item=item)
    return lists

def search_links(nume='', an='', sezon='', episod='', fisier='', manual='', item=None):
    subs_list = []
    if __addon__.getSetting("scrap") == "0":
        ...
        #log(continuturl)
        first_search = re.compile('class="post_title.+?<a href="(.+?)".+?me">(.+?)<', re.IGNORECASE | re.DOTALL).findall(continuturl)
        if first_search:
            selected = []
            if len(first_search) > 0:
                if len(first_search) > 1:
                    dialog = xbmcgui.Dialog()
                    sel = dialog.select("RegieLive",[x[1] for x in first_search])
                else:
                    sel = 0
                if sel >= 0:
                    if sezon == "" or episod == "":
                        ...
                        continuturl = get_search('%ssezonul-%s/episodul-%s/' % (first_search[sel][0], sezon.lstrip("0"), episod.lstrip("0")))
                    regex = '''<div class="col-md-12 nume.+?>(.+?)<.+?<ul class="rating_list_subtitrare.+?(?: |title="Nota (.+?) d).+?<div class="col-md-3 descarca.+?<a href="(.+?)"'''
                    search = re.compile(regex, re.IGNORECASE | re.DOTALL).findall(continuturl)
                    #with open(xbmc.translatePath(os.path.join('special://temp', 'files.py')), 'wb') as f: f.write(repr(search))
                    if len(search) > 0:
                        for item_search in search:
                            ...
                            selected.append({'SeriesSeason': (item['season']), 'SeriesEpisode': item['episode'], 'LanguageName': 'Romanian',
                                    'promo': '', 'SubFileName': (item_search[0].strip()), 'SubRating': rate,
...
def Download(link, url, format, stack=False):
    ...
    for file in files:
        ...
        if __addon__.getSetting("scrap") == "1" and __addon__.getSetting("nopop") == "false":
...
def get_search(url):
    ...
    try:
        continut = s.get(url, headers=headers)
