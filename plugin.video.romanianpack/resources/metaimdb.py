...
class window(xbmcgui.WindowDialog):
    ...
    def get_n(self, nameorig, link):
        ...
        results = self.searchmovie(nume, an, nameorig, link, nume2)
        ...
        if len(results) > 0:
            ...
            if sel >= 0:
                ...
                try: trailer = self.get_data(imdb_trailer, content)[0]
                except: trailer = ''
                ...
                self.buttont = xbmcgui.ControlButton(500, 670, 100, 30, 'Trailer', focusedColor='0xFFFFFF00', focusTexture='')
    ...
    def onAction(self, action):
        ...
        if action.getId() == 92 or action.getId() == 10 or action.getId() == 11:
            self.close()
        if self.trailer or self.traktwatch:
            ...
            if (action.getId() == 2 or action.getId() == 4) and not (self.getFocus() == (self.buttontrakt if self.traktwatch else self.buttont if self.trailer else self.button)):
                    self.setFocusId(self.getFocusId() + (1 if self.trailer else 2))
